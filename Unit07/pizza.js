...
document.querySelector("#tubular").addEventListener("click", () => pizza.crust = 'totally tubular')
...
document.querySelector("#shell").addEventListener("click", () => pizza.size = 'one shell of a')

//Toppings add and remove
function toppingAdder (toppingID, toppingName) {
    if (toppingID.checked == true && pizza.topping.includes(toppingName) == false) {
        pizza.topping.push(toppingName)
        } else if (toppingID.checked == false && pizza.topping.includes(toppingName) == true) {
...
//Buttons
document.querySelector("#readOrder").addEventListener("click",  () => readOrder());
document.querySelector("#shopList").addEventListener("click",  () => sayTopping());
